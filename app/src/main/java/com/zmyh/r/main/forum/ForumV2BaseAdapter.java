...
public class ForumV2BaseAdapter extends BaseAdapter {
    ...
    public ForumV2BaseAdapter(Context context) {
    ...
    public ForumV2BaseAdapter(Context context, List<DynamicObj> list) {
        this.itemList = list;
        this.context = context;
        inflater = (LayoutInflater) context
                .getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if (convertView == null) {
            convertView = inflater.inflate(R.layout.forum_list_item_v3, null);
        }

        DynamicObj dynamicObj = itemList.get(position);
        setContent(convertView, dynamicObj);
        setUserPic(convertView, dynamicObj);
    ...
    private void setUserPic(View view, final DynamicObj obj) {
...
//                    intent.putExtra("userId", obj.getPoster().getId());
//                    context.startActivity(intent);
                    Bundle b = new Bundle();
                    b.putString("id", obj.getPosterId());
    ...
    private void setPicList(View view, DynamicObj obj) {
        ...
        switch (pisList.size()) {
            case 1:
                ...
                width = WinTool.dipToPx(context, 90);
            ...
            case 4:
                w = WinTool.dipToPx(context, 80);
                width = WinTool.dipToPx(context, 175);
            ...
            default:
                w = WinTool.dipToPx(context, 80);
                width = WinTool.dipToPx(context, 260);
    ...
    private void setContent(View view, DynamicObj obj) {
        ...
        if (!DynamicObjHandler.isWatch(context, obj)) {
            userName.setTextColor(ColorBox.getColorForID(context, R.color.title_bg));
            time.setTextColor(ColorBox.getColorForID(context, R.color.black));
            title.setTextColor(ColorBox.getColorForID(context, R.color.black));
        } else {
            userName.setTextColor(ColorBox.getColorForID(context, R.color.title_bg));
            time.setTextColor(ColorBox.getColorForID(context, R.color.text_gray_04));
            title.setTextColor(ColorBox.getColorForID(context, R.color.text_gray_04));
        }
    }
